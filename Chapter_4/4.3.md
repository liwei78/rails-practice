...
如果在 has_one 中设置了 dependent ，当子资源失去该关联关系时，它也会被删除。如果不设置，一个子资源失去关系时，外键设置为 null。
...
在电商系统里，一个用户是有多个订单（Order）的，User 中适用的是 has_many 方法：
...
number 是订单的唯一编号，payment_state 是付款状态，shipment_state是发货状态。
...
一对多关系返回的，是 [CollectionProxy](http://api.rubyonrails.org/classes/ActiveRecord/Associations/CollectionProxy.html) 实例。适用该实例中的方法可以实现一对多关系的添加和删除操作。
...
如果 variant 是一个未保存到数据库的实例，<< 执行的时候会自动将它保存，并且赋予它 product_id 值。这是一步完成的，只有一条 SQL。如果是：
...
会自动把3，4的 product_id 外键设置为 product.id。
...
这时，我们需要给 users 表增加一个字段：orders_count，当我们把一个 order 保存到一对多的关系中时，orders_count 会自动 +1，当把一个资源从关系中删除，该字段会 -1。如此我们不必去增加计算一个 user 有多少个orders，只需要读该字段就可以了。
