...
回滚之后，迁移停留在回滚到的那个位置的，schema 也会更新到那个位置时的状态。
...
要留意的是，invalid? 和 valid? 都会调用实例的校验，这在下一节会讲到。
...
数据查询，是 Rails 项目经常要做的操作，如何拿到准确的数据，已经优化查询，是我们要重点关注的。
...
在使用 update 更新记录的时候，会调用 Model 的 validates（校验） 和 callbacks（回调），保证我们写入正确的数据，这个数据是定义在 Model 中的，我们后面章节会介绍校验和回调。但是，update_all 会略过校验和回调，直接将数据写入到数据库中，如果数据库层面做了数据校验，会抛出数据库异常。
...
使用 delete 删除时，会跳过回调和校验，直接从数据库中删除，它可是是一个类方法，比如：
...
destroy 方法，会触发 model 中定义的校验和回调，保证我们正确的操作。它也可以是类方法和实例方法，用法和前面的一样。

需要说明，delete/delete_all 和 destroy/destroy_all 都可以作用在关系查询结果（ActiveRecord::Relation）上，删掉查找到的记录。

如果你不像真正从数据库中抹掉数据，而是给它一个删除标注，可以使用 [https://github.com/radar/paranoia](https://github.com/radar/paranoia) 这个 gem，他会给记录一个 deleted_at 时间戳，并且使用 `restore` 方法把它从数据库中恢复过来，或者使用 `really_destroy!` 将它真正的删除掉。
