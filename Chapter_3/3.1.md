...
`layouts` 里放的是布局文件。如果我们网站只有一种布局，那么一个 `application.html.erb` 就足够了。如果 `admin` 要使用完全不同的布局和引用自己的 css 等，就要新建一个了。
...
`yield` 方法可以让 Rails 使用我们的模板 `app/views/products/index.html.erb` 填充了布局（layout）。
