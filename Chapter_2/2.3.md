...
在我们定义资源的时候，有时候一个资源会有它的子资源，比如一个商品（product）会有多个评论（comments），当我们浏览商品页面的时候，也会把这些评论显示出来。
...
这时，我们可以为 comments 也增加一下 `scaffold`：

```
rails g scaffold comments title content:text
```
...
记得，我们应该删除 routes 中自动添加的 `resources :comments`，因为我们不需要在 `localhost:3000/comments` 下看到它，不是么？我们可以在每一个商品（Product）页面，比如：`http://localhost:3000/products/1` 中看到它了。
...
好了，本章结束了，本节的内容多来自 Rails 手册中的 [routes 章节]([https://github.com/rspec/rspec-rails#routing-specs](https://github.com/rspec/rspec-rails#routing-specs)，你也可以找到在 [这里](https://github.com/liwei78/rails-practice-code) 找到本章的代码。下一章，我们将开始完成 shop 的页面（views）代码，希望它可以让你更加了解 Rails。
